...
계약서 분석이 완료되면 PDF를 페이지별 청크로 분리해 계약서 ID와 함께 ChromaDB에
저장합니다. `계약서에 질문하기`는 별도의 LangGraph 워크플로를 사용합니다.
...
→ Ollama 근거 기반 답변 생성
→ 답변·계약서 페이지·법령 출처 반환
→ Spring Boot 질문 이력 저장
```

기존에 분석한 계약서는 첫 질문 시 PDF를 한 번만 다시 읽어 계약서 벡터 인덱스를
자동 생성합니다. 이후 질문부터는 저장된 계약서별 Vector DB를 사용합니다.
