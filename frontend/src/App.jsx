...
function Dashboard({ user, onLogout }) {
  ...
  const upload = async (event) => {
    ...
    try { const body = new FormData(); body.append("file", file); await api("/contracts", {method:"POST", body}); await load(); }
    catch(e) { setError(e.message); } finally { setBusy(false); event.target.value = ""; }
  };
  const analyze = async (contract) => {
    setSelected(contract); setResult(null); setBusy(true); setError("");
    try { setResult(await api(`/contracts/${contract.id}/analysis`, {method:"POST"})); await load(); }
  ...
  return <div className="app-shell"><header><div><p className="brand-logo">LAW<span>Z</span>IC</p><h2>계약서 분석실</h2></div><div className="user"><span>{user?.name ?? user?.email}님</span><button className="secondary" onClick={onLogout}>로그아웃</button></div></header>
    ...
    <section className="card list"><h3>분석 이력</h3>{contracts.length === 0 ? <p className="empty">업로드한 계약서가 없습니다.</p> : contracts.map(c => <article className="contract" key={c.id}><div><strong>{c.filename}</strong><small>{new Date(c.uploadedAt).toLocaleString("ko-KR")} · {c.status}</small></div><button onClick={() => analyze(c)} disabled={busy}>분석</button></article>)}</section>
